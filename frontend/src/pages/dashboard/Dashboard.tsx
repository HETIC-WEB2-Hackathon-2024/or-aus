...
import { BookMarked, Heart, Pin, Send, Target } from "lucide-react";
        ...
        <div className="flex min-h-screen w-full flex-col">
            <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
                ...
                <div className="grid gap-4 md:grid-cols-2 md:gap-8 lg:grid-cols-4">
                    <StatisticsCard
                        title={
                            dashboardData
                                ? `Nouvelles offre ${
                                      dashboardData?.[StatsEnum.Commune]
                                          ?.commune
                                          ? `à ${
                                                dashboardData[StatsEnum.Commune]
                                                    ?.commune
                        ...
                        main_data={
                            dashboardData?.[StatsEnum.Commune]?.current_month ||
                            0
                        }
                        comparison_data={
                            dashboardData?.[StatsEnum.Commune]
                    ...
                    <StatisticsCard
                        title={
                            dashboardData
                                ? `Nouvelles offre ${
                                      dashboardData?.[StatsEnum.Secteur]
                                          ?.secteur
                                          ? `en ${
                                                dashboardData[StatsEnum.Secteur]
                                                    ?.secteur
                        ...
                        main_data={
                            dashboardData?.[StatsEnum.Secteur]?.current_month ||
                            0
                        }
                        comparison_data={
                            dashboardData?.[StatsEnum.Secteur]
                    ...
                    <StatisticsCard
                        ...
                        isError={isError}
                        icon={
                            <BookMarked className="h-6 w-6 text-muted-foreground stroke-primary" />
                        }
                    />
                    <StatisticsCard
                        title={StatsTitleEnum.Candidature}
                        main_data={
                            dashboardData?.[StatsEnum.Candidature]
                                ?.current_month || 0
                        }
                        comparison_data={
                            dashboardData?.[StatsEnum.Candidature]
                ...
                <div className="grid gap-4 md:gap-8 lg:grid-cols-2 xl:grid-cols-3 h-auto">
                    <GraphicCard
                        data={dashboardData?.[StatsEnum.Graphique] || []}
                    />
                    <Card
                        ...
                        <CardContent className="grid gap-3">
                            {favoriteData?.length === 0 && (
                                <LoadingCard
                                    isError={true}
                                    errorTitle={
                                        "Commencez votre recherche dans la page Offres"
                                    }
                                />
                            )}
                                    ...
                                    return (
                                        ...
                                        >
                                            <CardTitle className="text-md mb-1">
                                                {favorite.titre_emploi}
                                            </CardTitle>
                                            <CardDescription className="text-sm">
                                                {favorite.entreprise} -{" "}
                                                {favorite.lieu} (
                                                {favorite.code_region})
                                            </CardDescription>
                                        </Card>
                                    );
                                })
