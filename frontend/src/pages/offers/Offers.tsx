...
export default function Offers() {
  ...
  useEffect(() => {
    ...
    if (periodStart && periodEnd) {
      ...
      if (noDateQueryFilters.period_start)
        delete noDateQueryFilters.period_start;
  ...
  const addContractTypeFilter = (e: React.FormEvent<HTMLButtonElement>) => {
    if (e.currentTarget.getAttribute("data-state") == "unchecked") {
      ...
      const updatedFilterContractTypeFilter = queryFilters.type_contrat?.filter(
        (type) => type !== e.currentTarget.id
      );
      if (
        updatedFilterContractTypeFilter &&
        updatedFilterContractTypeFilter.length > 0
      ) {
        setQueryFilters({
          ...queryFilters,
          type_contrat: updatedFilterContractTypeFilter,
        });
      } else {
        const noContractTypeFilter = queryFilters;
        if (noContractTypeFilter.type_contrat)
          delete noContractTypeFilter.type_contrat;
