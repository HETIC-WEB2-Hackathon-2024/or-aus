...
import {
  ...
  TCandidatEmail,
  TCandidatId,
} from "../core/candidat/domain/Candidat";
import {
    ...
    ISecteurRepository
{
  ...
  async removeFavorite(input: RemoveFavoriteDto): Promise<void> {
    ...
    try {
      ...
      const userHasThisOffer = userFavorites.some(
        (offer) => offer.id === input.offre_id
      );
      if (!userHasThisOffer) throw new Error("Favorite doesn't exist");

      await client.query(
        "DELETE FROM favorite AS f WHERE f.offre_id = $1 AND f.candidat_id = $2",
        [input.offre_id, input.user_id]
      );
    } finally {
      client.release();
    }
  ...
  async getContractTypes(): Promise<TContract[]> {
    ...
    try {
      const results = await client.query(
        "SELECT DISTINCT contrat FROM offre",
        []
      );
      return results.rows;
    } finally {
  ...
  async getSecteursDistinct(): Promise<Secteur[]> {
    const client = await this._pool.connect();
    try {
      const results = await client.query<Secteur>(
        "SELECT DISTINCT secteur FROM secteur",
        []
      );
      return results.rows;
    } finally {
