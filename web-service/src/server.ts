...
import {
  GetCandidatInfoMiddleware,
  RequestWithUserInfo,
} from "./core/candidat/controllers/GetCandidatInfoMiddleware";
...
export async function main(): Promise<void> {
  ...
  // Candidat
  const getCandidatInfoUseCase = new GetCandidatInfoUseCase(
    auth0Repository,
    postgreRepository
  );
  const getCandidatInfoMiddleware = new GetCandidatInfoMiddleware(
    getCandidatInfoUseCase
  );
  ...
  const getCandidatCandidaturesCountUseCase =
    new GetCandidatCandidaturesCountUseCase(postgreRepository);
  const getCandidatCommuneOffersStatsUseCase =
    new GetCandidatCommuneOffersStatsUseCase(postgreRepository);
  const getCandidatSecteurOffersStatsUseCase =
    new GetCandidatSecteurOffersStatsUseCase(postgreRepository);
  const getCandidatFavoriteCountUseCase = new GetCandidatFavoriteCountUseCase(
    postgreRepository
  );
  ...
  const getContractTypesUseCase = new GetContractTypesUseCase(
    postgreRepository
  );
  const getContractTypesController = new GetContractTypesController(
    getContractTypesUseCase
  );
  ...
  offersRouter
    .route("/v1/offres/favorite")
    .delete(getCandidatInfoMiddleware.handle, removeFavoriteController.handle);
  offersRouter
    .route("/v1/offres/favorite")
    .get(getCandidatInfoMiddleware.handle, getFavoriteController.handle);
  offersRouter
    .route("/v1/offres/favorite")
    .post(getCandidatInfoMiddleware.handle, addFavoriteController.handle);
  offersRouter
    .route("/v1/offres/contractTypes")
    .get(getContractTypesController.handle);
